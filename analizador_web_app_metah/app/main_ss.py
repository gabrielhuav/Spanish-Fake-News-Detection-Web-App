...
app = Flask(__name__)
MODELO_PATH = './modelo_ss/' # Apunta a la nueva carpeta del modelo

# --- Cargar todos los componentes del modelo al iniciar ---
print("Cargando modelo de Búsqueda Dispersa...")
try:
    vectorizer = load(os.path.join(MODELO_PATH, 'vectorizer.joblib'))
    selector = load(os.path.join(MODELO_PATH, 'selector_caracteristicas_ss.joblib'))
    solucion = np.load(os.path.join(MODELO_PATH, 'modelo_ss_solucion.npy'))
    pesos = np.load(os.path.join(MODELO_PATH, 'modelo_ss_pesos.npy'))
    umbrales = np.load(os.path.join(MODELO_PATH, 'modelo_ss_umbrales.npy'))
    ...
    print("❌ Error fatal: Faltan archivos del modelo en la carpeta 'app/modelo_ss/'.")
    exit()
