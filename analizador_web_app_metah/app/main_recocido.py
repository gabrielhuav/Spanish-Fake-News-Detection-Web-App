...
app = Flask(__name__)
MODELO_PATH = './modelo_recocido/'

# --- Cargar todos los componentes del modelo al iniciar ---
print("Cargando modelo de Recocido Simulado...")
try:
    vectorizer = load(os.path.join(MODELO_PATH, 'vectorizer.joblib'))
    selector = load(os.path.join(MODELO_PATH, 'selector_caracteristicas.joblib'))
    solucion = np.load(os.path.join(MODELO_PATH, 'modelo_recocido_solucion.npy'))
    pesos = np.load(os.path.join(MODELO_PATH, 'modelo_recocido_pesos.npy'))
    umbrales = np.load(os.path.join(MODELO_PATH, 'modelo_recocido_umbrales.npy'))
    ...
    print("❌ Error fatal: Faltan archivos del modelo en la carpeta 'app/modelo_recocido/'.")
    exit()
...
def predecir_texto(texto_nuevo: str):
    ...
    vector_reducido = selector.transform(vector_bow).toarray() # Aseguramos que sea una matriz densa
    
    # --- CORRECCIÓN CLAVE: Seleccionar la primera (y única) fila ---
    # Esto asegura que las dimensiones sean (500,) en lugar de (1, 500)
    caracteristicas_activas = vector_reducido[0, solucion]
    
    x_binario = (caracteristicas_activas >= umbrales).astype(int)
    
    # Ahora el producto punto funcionará: (500,) . (500,)
